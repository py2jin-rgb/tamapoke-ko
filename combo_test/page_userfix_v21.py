...
# This runs after page_quality_v20.py, so it patches the exact source that is
# compiled for the one-click web installer.
...
# ---------------------------------------------------------------------------
# 1) Mole/Pikachu whack game: make every successful hit clearly audible.
#    Keep cues short so rapid taps remain responsive, but give them a stronger
#    low attack + rising "pop" character for tactile feedback.
# ---------------------------------------------------------------------------
aud = once(
    aud,
    'static const Note N_MOLE_H1[]    = {{740, 28}, {988, 35}};',
    'static const Note N_MOLE_H1[]    = {{190, 18}, {720, 28}, {1040, 42}};',
    'mole hit sound 1')
aud = once(
    aud,
    'static const Note N_MOLE_H2[]    = {{831, 28}, {1109, 35}};',
    'static const Note N_MOLE_H2[]    = {{220, 18}, {830, 28}, {1240, 42}};',
    'mole hit sound 2')
aud = once(
    aud,
    'static const Note N_MOLE_H3[]    = {{932, 28}, {1245, 38}};',
    'static const Note N_MOLE_H3[]    = {{260, 18}, {930, 28}, {1480, 44}};',
    'mole hit sound 3')

# The note table originally describes these three sounds as two-note cues.
# Update lengths so the third impact note is actually played.
aud = once(
    aud,
    '{N_MOLE_H1, 2}, {N_MOLE_H2, 2}, {N_MOLE_H3, 2}, {N_MOLE_MISS, 2},',
    '{N_MOLE_H1, 3}, {N_MOLE_H2, 3}, {N_MOLE_H3, 3}, {N_MOLE_MISS, 2},',
    'mole note table lengths')
audio.write_text(aud, encoding='utf-8')


# ---------------------------------------------------------------------------
# 2) Standby clock: remove the large rounded rectangle behind the Pokemon.
#    Keep only the Pokemon and tiny ground line directly on the sky layer.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3) 151 quiz: larger Pokemon + randomized question order on every attempt.
#    The saved quiz level still controls difficulty/progress, but the target
#    Pokemon is freshly randomized each question (including after a wrong answer)
#    and the correct choice slot is randomized too.
# ---------------------------------------------------------------------------
src = once(
    src,
    '  quizTargetDex = QUIZ151_ORDER[quizLevel - 1];',
    '''  const uint16_t previousDex = quizTargetDex;\n  do { quizTargetDex = (uint16_t)(1 + random(151)); } while (quizTargetDex == previousDex);''',
    'quiz random target')

src = once(
    src,
    '  quizCorrectSlot = (uint8_t)((quizLevel * 7u) % 3u);',
    '  quizCorrectSlot = (uint8_t)random(3);',
    'quiz random correct slot')

# Increase the main question sprite from max scale 3 to 4 and give it a little
# more vertical room before the answer buttons.
...
# Mark the exact firmware in Settings/About without changing save layout.
src, n = re.subn(
    r'#define FW_VERSION "2\.0-ko-combo-qualitygames-final"',
    '#define FW_VERSION "2.1-ko-combo-userfix"',
    src,
    count=1)
if n != 1:
    raise SystemExit('v2.1 marker not found: firmware version')

ino.write_text(src, encoding='utf-8')


# ---------------------------------------------------------------------------
# Recompile the combo firmware so the installer receives these source changes.
# ---------------------------------------------------------------------------
...
subprocess.run([
    ...
    '--build-path', str(build), str(root)
], check=True)
if not (build / 'TamaPoke.ino.bin').is_file():
    raise SystemExit('v2.1 combo firmware binary missing')
print('v2.1 combo firmware compiled successfully')


# ---------------------------------------------------------------------------
# Installer page labels / change notes.
# ---------------------------------------------------------------------------
