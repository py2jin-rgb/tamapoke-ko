...
# and the real-hardware standby clock overlap fix. ①~④ are untouched.
subprocess.run(['python3','combo_test/ota_v25.py'],check=True)
subprocess.run(['python3','combo_test/volume_ota_v26.py'],check=True)
subprocess.run(['python3','combo_test/clock_layout_fix_v27.py'],check=True)

src=Path('build_ota/TamaPoke.ino.bin')
if not src.is_file(): raise SystemExit('OTA volume clockfix pipeline binary missing')
data=src.read_bytes()
sha=hashlib.sha256(data).hexdigest(); short=sha[:12]
name=f'tamapoke-ko-cheonjiin-ota27-volume-clockfix-{short}.bin'
site=Path('site'); fw=site/'firmware'; fw.mkdir(parents=True,exist_ok=True)
shutil.copy2(src,fw/name)

manifest={
  'name':'TamaPoke KO Cheonjiin OTA Volume Clock Fix 2.7',
  'version':f'2.7-ko-combo-ota-volume-clockfix-{short}',
...
(site/'ota-info.txt').write_text(
    f'OTA Volume Clock Fix 2.7 SHA256: {sha}\nFile: {name}\nBuild: 270\nVolume: 0/25/50/75/100 percent, NVS persisted, PCM amplitude scaled\nClock: standby Pokemon scale/ground/date overlap fix\nManifest: ota-latest.json\n',encoding='utf-8')
...
# Upgrade only ⑤ to 2.7 and advertise both persistent volume and the standby
# clock real-hardware layout correction.
html=html.replace('📶 NEW · OTA 2.5','🔊 NEW · OTA + VOLUME 2.7',1)
html=html.replace('manifest-ota.json?v=ota25','manifest-ota.json?v=ota27clockfix',1)
html=html.replace('📶 천지인 OTA 2.5 설치','🔊 천지인 OTA + Volume 2.7 설치',1)
needle='<p class="ok">✓ OTA 전용 슬롯에 기록 후 정상 완료시에만 재부팅</p>'
extra=needle+'<p class="ok">✓ 소리 0 / 25 / 50 / 75 / 100% 조절 · 재부팅 후에도 저장</p><p class="ok">✓ 시계모드 포켓몬 크기/위치 조정 · 시간/날짜 겹침 수정</p>'
...
for must in ('⑤ 천지인 Battle · Battery + Wi-Fi 무선 업데이트','manifest-ota.json?v=ota27clockfix','OTA + VOLUME 2.7','0 / 25 / 50 / 75 / 100%','시계모드 포켓몬 크기/위치 조정'):
    if must not in html: raise SystemExit('OTA volume clockfix installer invariant missing: '+must)
print('OTA Volume Clock Fix 2.7 publish complete; single installer ⑤ kept:',name,sha)
